...
def split_prompt_for_long_video(user_prompt: str) -> Dict[str, str]:
    ...
    """
    try:
        system_prompt = """你是一个提示词分析专家。你的任务是将用户提供的视频理解提示词拆分为片段理解提示词和整合提示词。
片段理解提示词将会被交给多个片段理解模型，整合提示词将会被交给一个整合模型。

1. **片段理解提示词** - 用于理解单个视频片段，应包括：
   - 视频内容的特殊性说明（如"这是实验室操作视频"、"演示者讲的话是重点"等）
   - **固定提醒**: 必须包含提醒"语音识别结果对应完整视频，提供给你是便于了解全貌，但请你只描述你获得的视频片段中的内容"
   - 内容理解的注意事项（如"注意细节"、"结合语音识别理解"等）
   - 片段级别的输出要求（如"提供材料清单"、"列出操作步骤"、"提出待澄清的问题"、"操作步骤中适当分段，每一段包含"目的"和"操作"两个层级"等。）
   - 通用格式要求（如"时间戳格式"、"中文输出"、"纯文本输出"、"使用或不使用Markdown"等）

2. **整合提示词** - 用于整合所有片段结果，应包括：
   - 整合级别的输出要求（如"生成标题"、"写摘要"、"提取关键词"、"操作步骤中适当分段，每一段包含"目的"和"操作"两个层级"等）
   - 最终文档的结构要求（如"包含标题、摘要、关键词、材料、步骤、问题"等。）
   - 通用格式要求（如"时间戳格式"、"中文输出"、"纯文本输出"、"使用或不使用Markdown"等）

**重要原则**：
- 某些格式要求（如时间戳格式、输出格式）需要同时出现在两个提示词中
- 某些输出内容要求只属于整合阶段（如标题、摘要、关键词通常在整合时生成）
- 某些输出内容要求可能同时需要（如材料清单、操作步骤、待澄清的问题通常需要在片段理解中提取，在整合时汇总）
- 用户的整合要求可能包括标题、摘要、关键词、材料清单、操作步骤、待澄清的问题等任意组合
...
只返回JSON，不要添加其他文字说明。"""
        ...
        user_analysis_prompt = f"""{user_prompt}"""
